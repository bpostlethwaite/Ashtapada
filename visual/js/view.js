...
var View = {
    ...
    init: function(opts) {
        ...
        //   center div hack
        var grid = $('#draw_area')
          , width = '-'+Math.round(this.numCols * this.nodeSize / 2)+'px'
          , height = '-' + Math.round(this.numRows * this.nodeSize / 2)+'px'
        grid.css({
            position:'absolute'
          , left: '50%'
          , top: '50%'
          , margin: height + ' 0 0 ' + width
        })
    },
    /**
    ...
    moveUnit: function(id, toX, toY, cost) {
        if (!cost) cost = 1000
        var unit = this.units[id]
        var coord = this.toPageCoordinate(toX, toY);
        unit.animate({x:coord[0], y:coord[1]}, cost)
    },
    newUnit: function (id, color, gridX, gridY) {
        var coord = this.toPageCoordinate(gridX, gridY);
        this.nodeStyle.unit.fill = color
        this.units[id] = this.paper.rect(
            coord[0],
            coord[1],
            this.nodeSize,
            this.nodeSize
        ).attr(this.nodeStyle.unit)
    },
    /**
